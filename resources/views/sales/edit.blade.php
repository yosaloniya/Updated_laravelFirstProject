        ...
        <div class="col-md-12 grid-margin stretch-card mx-auto sales-form">
            <div class="card">
                <div class="card-body main-row-alert">
                    <div class="row">
                        <div class="col-8">
                            <h4 class="card-title text-dark">Edit Order details</h4>
                        </div>
                    ...
                    <form class="forms-sample create-user pt-2">
                        <input type="number" value="{{ $data['id'] }}" class="data-id" hidden>
                        <div class="row">
                            <div class="form-group col-sm-4">
                                <label for="exampleInputPassword1">Order Number :</label>
                                <input type="text" value="{{ $data->order_no }}" class="form-control order_no"
                                    name="order_no" placeholder="Enter Order No." required disabled>
                            </div>
                            <div class="form-group col-sm-4">
                                <label for="date">Date :</label>
                                <input type="text" class="form-control"  value="{{ date('d/m/Y', strtotime($data->date)) }}" name="date" placeholder="Enter Date"
                                    id="date">
                            </div>
                            <div class="form-group col-sm-4">
                                <label for="exampleInputPassword1">Customer Name :</label>
                                <select class="form-control customer_id productselect" name="customer_id" required>
                                    <option value="" selected disabled>Choose Customer</option>
                                    @foreach ($customer as $cust)
                                        <option
                                            value="{{ $cust->id }}"{{ $data->customer_id == $cust->id ? 'selected' : '' }}>
                                            {{ $cust->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                                ...
                                <div class="row productdata" id="">
                                    <div class="form-group col-sm-3">
                                        ...
                                        <select class="form-control sku-select productselect sku-select" name="sku"
                                            ...
                                            @foreach ($product as $pr)
                                                <?php
                                                if (isset($pr1['sku']) && $pr->id == $pr1['sku']) {
                                                    echo '<option value=' . $pr->id . ' data-name=' . $pr->name . 'data-price=' . $pr->t_price . ' data-sub_sku="' . $alldata::getproductname1($pr1['sub_sku']) . '" data-sub_sku="' . $alldata::getproductnamesku($pr1['sub_sku']) . '" selected>' . $pr->sku . '</option>';
                                                } else {
                                                    echo '<option value=' . $pr->id . ' data-name=' . $pr->name . 'data-price=' . $pr->t_price . '>' . $pr->sku . '</option>';
                                                }
                                                ?>
                                    ...
                                    <div class="form-group asd col-sm-3">
                                        ...
                                        <select class="form-control sub-sku productselect subsku-select" name="sub_sku"
                                            disabled>
                                            <option value="{{ $pr1['sub_sku'] }}" selected>
                                                {{ $alldata::getproductnamesku($pr1['sub_sku']) }}</option>
                                        </select>
                                    </div>
                                    <div class="form-group col-sm-4 d-none">
                                        <label for="exampleInputPassword1">Product Name :</label>
                                        <?php
                                        if (!empty($alldata::getproductname1($pr1['sub_sku']))) {
                                            echo '<input type="text" value=' . $alldata::getproductname1($pr1['sub_sku']) . ' class="form-control product-name" name="product_id" data-p_id="' . $pr1['p_id'] . '" placeholder="Enter Product Name" required disabled>';
                                        } else {
                                            echo '<input type="text" value=' . $alldata::getproductname($pr1['sku']) . ' class="form-control product-name" name="product_id" data-p_id="' . $pr1['p_id'] . '" placeholder="Enter Product Name" required disabled>';
                                        }
                                        ?>
                                    </div>
                                    <div class="form-group col-sm-4">
                                        <div class="row">
                                            <div class="col-sm-6 location-div">
                                                ...
                                                <p class="form-control location location-box"></p>
                                            </div>
                                            <div class="col-sm-6 location-div">
                                                <label for="exampleInputPassword1">Total Stock :</label>
                                                <p class="form-control stock stock-box"></p>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="form-group col-sm-2">
                                        ...
                                        <div class="row">
                                            <div class="col-sm-12">
                                            <input type="number" data-old_qty="<?php $var = $pr1['qty'];
                                            echo abs($var); ?>"
                                                    value="<?php $var = $pr1['qty'];
                                                    echo abs($var); ?>" class="form-control qty" name="qty"
                                                    placeholder="Enter Quantity" onkeyup="calculatePrice()">
                                            </div>
                        ...
                        <div class="form-group col-sm-12">
                                    ...
                                    <input type="text" value="{{ $data->description }}" class="form-control desc-description" name="description"
                        ...
                        <button class="btn btn-primary mr-2 mt-5 addselasdata" type="submit">Update</button>
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/select2/4.0.13/js/select2.min.js"></script>
    <script>
        ...
        $(document).ready(function() {
            ...
            $('form').submit(function(e) {
                ...
                $('.productdata').each(function() {
                    ...
                    let OldQty = $(this).find('.qty').data('old_qty');
                    let newQty = +qty;
                    newQty = OldQty ? OldQty - qty : -1 * newQty;
                    ...
                    $.ajax({
                        ...
                        <div class="alert alert-success fade-in alert-dismissible alertmsgnew" role="alert">\n\
                            ...
                            <div class="alert alert-danger fade-in alert-dismissible alertmsgnew" role="alert">\n\
                                <button type="button" class="close" data-dismiss="alert">\n\
                                    <i class="fa fa-times"></i></button><strong>Error ! </strong> Order not Updated!\n\
                        </div>`;
                                $(".main-row-alert").prepend(errorMessage);
                                setTimeout(() => {
                                    $('.alertmsgnew').fadeOut('slow');
            ...
            $('.addData').click(function() {
                ...
                let str =
                    '<hr class="bg-secondary hr-line mt-3 mb-4"> <div class="row productdata"> <div class="form-group col-sm-3"> <label for="exampleInputPassword1">Choose SKU :</label>\n\
                                                       ...
                                                       </select> </div><div class="form-group col-sm-4 d-none"><label for="exampleInputPassword1">Product Name :</label>\n\
                                            ...
                                            <div class="row">\n\
                                                ...
                                                <div class="col-sm-6 location-div">\n\
                                                    ...
                                                    <div class="form-group col-sm-2"><label for="exampleInputPassword1">Size(mtr.) :</label>\n\
                                                  <div class="row"><div class="col-sm-10"><input type="number" class="form-control qty" name="qty" placeholder="Enter Size" onkeyup="calculatePrice()"></div><div class="col-sm-2"><a class="btn btn-sm btn-light" onClick="remove_section(this)"><i class="fa fa-close"></i></a></div></div></div></div>';
            ...
            $(document).on('change', '.productdata:last-child .subsku-select', function() {
                var id = $(this).val();
                let url = "{{ url('product/price') }}";
                let d = $(this);
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        d.closest('.productdata').find('.stock-box').text(data[0].qty + " " +
                            "mtr.");
                    },
                    error: function(xhr, status, error) {
                        console.error(xhr.responseText);
        ...
        function SubSkuShow() {
            let subSku = $('.sku-select option:selected').data('sub_sku');
            $('.sub-sku').html(`<option value="${subSku}" selected>${subSku}</option>`);
            // console.log(subSku, "fghj");
        }
    </script>
@endsection
